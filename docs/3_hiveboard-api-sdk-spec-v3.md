...
**Changes from v1:** This version incorporates (1) project support across all endpoints, (2) Cost Explorer API endpoints, (3) `task.llm_call()` and `agent.llm_call()` SDK methods with `tokens_in`/`tokens_out`/`cost` as optional per addendum, (4) heartbeat payload and queue provider callbacks, (5) plan tracking with `task.plan()` and `task.plan_step()`, (6) work pipeline observability (`agent.queue_snapshot()`, `agent.todo()`, `agent.scheduled()`), (7) agent self-reported issues (`agent.report_issue()`, `agent.resolve_issue()`), (8) `GET /v1/agents/{agent_id}/pipeline` endpoint, (9) `cost_threshold` alert type, (10) payload convention awareness throughout, (11) `GET /v1/llm-calls` dedicated endpoint per addendum, and (12) `metric` + `group_by` parameters on `GET /v1/metrics` per addendum. See Appendix B for the complete change summary.
...
| `metric` | string | `null` (all) | **New in v2.** Focus response on a specific metric: `"cost"`, `"tokens"`, `"tasks"`, `"duration"`. When set, `summary` and `timeseries` include only the relevant metric's fields. When `null`, all metrics are returned (default v1 behavior). |
| `group_by` | string | `null` (none) | **New in v2.** Group results by a dimension: `"agent"`, `"model"`, `"agent_model"`. When set, response includes a `groups` array with per-group breakdowns. Requires `metric` to be set. See grouped response format below. |
...
**Grouped response format (when `group_by` is set):**

When `group_by` is provided with a `metric`, the response shape changes to include a `groups` array. The `summary` remains the workspace-wide total; `groups` contains per-dimension breakdowns.

Example: `GET /v1/metrics?metric=cost&group_by=model&range=24h`
...
```json
{
  ...
  "summary": {
    ...
    "avg_cost_per_task": 0.046,
    "llm_call_count": 568,
    "total_tokens_in": 852000,
    "total_tokens_out": 113600
  },
  "groups": [
    {
      "model": "claude-sonnet-4-20250514",
...
Example: `GET /v1/metrics?metric=tokens&group_by=agent&range=7d`
...
```json
{
  ...
  "metric": "tokens",
  "summary": {
    "total_tokens_in": 5964000,
    "total_tokens_out": 795200,
    "llm_call_count": 3976
  },
  "groups": [
    {
...
**`group_by` validation:** If `group_by` is provided without `metric`, the server returns a `400` error:

```json
{
  "error": "invalid_parameter",
  "message": "group_by requires a metric parameter (one of: cost, tokens, tasks, duration).",
  "status": 400
}
```

**Relationship to `GET /v1/cost`:** The Cost Explorer endpoints (Sections 4.7–4.9) remain the canonical interface for rich cost analysis with dedicated response shapes. The `group_by` on `GET /v1/metrics` provides a lighter-weight inline grouping for dashboard widgets and quick queries. Both return consistent numbers from the same underlying aggregation.
...
### 4.10 `GET /v1/llm-calls`

**New in v2. Dashboard screen:** Cost Explorer — Recent LLM Calls, Activity Stream (filtered)

Dedicated endpoint for querying LLM call events. Returns events where `payload.kind = "llm_call"`, ordered newest-first by default. Response includes per-call detail and aggregated totals for the matching result set.

This endpoint is the addendum-specified equivalent to `GET /v1/cost/calls` (Section 4.8), exposed under a more discoverable path. Both endpoints return the same underlying data; `GET /v1/llm-calls` adds the `totals` summary object.
...
| `project_id` | string | `null` (all) | Scope to this project. |
| `agent_id` | string | `null` (all) | Filter by agent. |
| `model` | string | `null` (all) | Filter by model identifier. Exact match. |
| `task_id` | string | `null` (all) | Filter by task. |
| `environment` | string | `null` (all) | Filter by environment. |
| `time_range` | string | `"24h"` | Time range shorthand: `"1h"`, `"6h"`, `"24h"`, `"7d"`, `"30d"`. Takes precedence over `since`/`until` when provided. |
| `since` | ISO 8601 | `null` | Start of time range. Used when `time_range` is not set. |
| `until` | ISO 8601 | `null` | End of time range. Used when `time_range` is not set. |
| `sort` | string | `"newest"` | Sort: `"newest"`, `"oldest"`, `"cost"`, `"tokens"`. |
| `limit` | integer | 50 | Max items. Max: 200. |
| `cursor` | string | `null` | Pagination cursor. |
...
The `totals` object aggregates across the **full filtered result set** (not just the current page). If no matching events exist, `totals` values are all `0`. Fields are extracted from the `custom` event's `payload.data`; missing fields are `null` in the response.

**`time_range` convenience parameter:** Converts to `since`/`until` server-side. `"24h"` becomes `since = now - 24h, until = now`. When both `time_range` and `since`/`until` are provided, `time_range` wins.

### 4.11 `GET /v1/agents/{agent_id}/pipeline`
...
### 4.21 `DELETE /v1/projects/{project_id}/agents/{agent_id}`
...
```python
task.llm_call(
    name="phase1_reasoning",          # required
    model="claude-sonnet-4-20250514", # required
    tokens_in=1500,                   # optional — feeds Cost Explorer
    tokens_out=200,                   # optional — feeds Cost Explorer
    cost=0.003,                       # optional — USD, feeds cost aggregation
...
| `tokens_in` | int | `None` | Input token count. Optional per addendum — not all LLM clients expose token counts. When provided, feeds Cost Explorer aggregation. |
| `tokens_out` | int | `None` | Output token count. Optional per addendum — same rationale as `tokens_in`. |
| `cost` | float | `None` | Pre-calculated cost in USD. The SDK does not maintain pricing tables — the developer or their LLM client provides this. Optional; when absent, the call appears in timelines but is excluded from cost aggregation totals. |
...
The SDK auto-generates the `summary` field from the parameters. When `tokens_in` and `tokens_out` are provided, format is: `"name → model (tokens_in in / tokens_out out, $cost)"`. When tokens are absent: `"name → model"`. When cost is absent the cost fragment is omitted.
...
Identical signature to `task.llm_call()` (including `tokens_in`, `tokens_out`, and `cost` being optional) but does not require an active task context. The resulting event has `task_id: null` and `project_id: null`.
...
**Behavior:** Emits a `custom` event with `payload.kind = "todo"` and `task_id: null` (agent-level). The `todo_id` field is critical — the `GET /v1/agents/{agent_id}/pipeline` endpoint (Section 4.11) groups TODO events by `todo_id` to derive current state.
...
**Behavior:** Emits a `custom` event with `payload.kind = "issue"` and `data.action = "resolved"`. The Pipeline endpoint (Section 4.11) uses the `action` field to determine which issues are still active.
...
| — | — | `GET /v1/metrics` | — | Summary bar, Charts, Grouped cost/token views (with `metric` + `group_by`) |
| — | — | `GET /v1/cost` | — | Cost Explorer |
| — | — | `GET /v1/cost/calls` | — | Cost Explorer (Recent Calls) |
| — | — | `GET /v1/cost/timeseries` | — | Cost Explorer (Chart) |
| — | — | `GET /v1/llm-calls` | — | Cost Explorer (Recent LLM Calls), Activity Stream |
...
| **`GET /v1/metrics`** | New fields: `llm_call_count`, `total_tokens_in`, `total_tokens_out` in summary and timeseries. **New params: `metric`, `group_by`** for inline grouped queries (e.g., cost by model). | 4.6 |
...
| **`GET /v1/llm-calls`** | **New endpoint.** Dedicated LLM call query with aggregated totals and `time_range` shorthand. Addendum-specified complement to `/v1/cost/calls`. | 4.10 |
| **`GET /v1/agents/{id}/pipeline`** | **New endpoint.** Work pipeline state (queue, TODOs, scheduled) | 4.11 |
| **Project endpoints** | **9 new endpoints.** CRUD + archive + agent management | 4.12–4.21 |
...
| **`task.llm_call()`** | **New method.** Structured LLM call tracking (task-scoped). `tokens_in`, `tokens_out`, `cost` are **optional** per addendum (not required). | 12.3 |
| **`agent.llm_call()`** | **New method.** Structured LLM call tracking (agent-level, no task context). Inherits optional `tokens_in`/`tokens_out`/`cost`. | 12.4 |
