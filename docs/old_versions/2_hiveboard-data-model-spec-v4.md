...
SELECT
    COALESCE(issue_id, summary) AS issue_key,
    summary,
    severity,
    category,
    action,
...
**Deduplication note:** Issues are grouped by `issue_id` when present, falling back to `summary` text. This means issues without an explicit `issue_id` are deduplicated by their summary — matching the addendum's intent that `issue_id` is optional with summary-based dedup as the fallback.
...
**Note on `group_by` for metrics:** The addendum proposed adding `group_by=model` and `group_by=agent_id` parameters to `GET /v1/metrics` for cost breakdowns. This use case is fully covered by the dedicated `/v1/cost` endpoints (which provide richer per-model and per-agent breakdowns with token detail). The `GET /v1/metrics` endpoint remains focused on operational metrics (task throughput, success rate, duration) with total cost as a summary figure.
