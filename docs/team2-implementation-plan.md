# Team 2 — Clients Implementation Plan

> **Scope:** HiveLoop SDK (Python) + Dashboard (HTML/CSS/JS)
> **Contract:** Codes against the API spec (`docs/3_hiveboard-api-sdk-spec-v3.md`)
> **Tests with:** Mock server / recorded responses (no real backend needed)
> **Shared types:** `from shared import EventType, Event, BatchEnvelope, ...`

---

## Internal Parallelism

Team 2 owns two deliverables that share no code. Two sub-tracks can work simultaneously:
...
> **Reference:** `docs/hiveboard-dashboard-v3.html` (the complete working prototype)
...
|-------|-----------|------------|----------|
| **C1.1** Transport | 6 | shared/ models | `_transport.py` — working batched HTTP transport |
| **C1.2** Core | 8 | C1.1 | `__init__.py`, `_agent.py` — init, agent, task, events |
| **C1.3** Decorators | 4 | C1.2 | `@track` decorator + context manager in `_agent.py` |
| **C1.4** Convenience | 9 | C1.2 | 9 methods across `_agent.py` (llm_call, plan, queue, todo, scheduled, issues) |
| **C1.5** Tests | 6 | C1.1–C1.4 | Full test suite in `tests/` |
| **C2.1** Shell | 6 | None | Empty dashboard chrome with layout + theme |
| **C2.2** Hive | 6 | C2.1 | Agent cards with all states + interactions |
| **C2.3** Center | 18 | C2.2 | Mission Control + Cost Explorer + Agent Detail |
| **C2.4** Stream | 6 | C2.1 | Activity stream with filters + live feed |
| **C2.5** Wiring | 6 | C2.1–C2.4 | API + WebSocket integration |

**Total: 75 sub-tasks across 10 sub-phases.**
...
|------|------|-------------------|
| Event Schema v2 | `docs/1_HiveBoard_Event_Schema_v2.md` | Section 6: all payload kind definitions |
| API + SDK Spec v3 | `docs/3_hiveboard-api-sdk-spec-v3.md` | Part B (Sections 8–15): full SDK spec. Part A (Sections 4–5): query + WebSocket endpoints |
| Dashboard v3 Prototype | `docs/hiveboard-dashboard-v3.html` | Complete working reference — open in browser |
| Dashboard Notes | `docs/hiveboard-dashboard-v3-notes.md` | Design decisions summary |
| Shared Types | `shared/models.py` | All Pydantic models both teams use |
| Shared Constants | `shared/enums.py` | EventType, Severity, PayloadKind, field limits |
| Sample Fixture | `shared/fixtures/sample_batch.json` | 22 events covering all 13 types + all 7 payload kinds |
