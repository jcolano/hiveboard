...
function navigateToSubPanel(panelName, subPanelName) {
    ...
    if (panelName === 'agents') {
        const hideSelector = (subPanelName === 'agents-list' || subPanelName === 'agents-tools');
        agentSelector.style.display = hideSelector ? 'none' : 'flex';
        if (!hideSelector) updateHeartbeatButton();
    }
...
function loadSubPanelData(panelName, subPanelName) {
    if (panelName === 'agents') {
        switch (subPanelName) {
            ...
            case 'agents-tools': loadTools(); break;
...
// Dynamic tool list -- fetched from GET /tools on first use
let _cachedTools = null;

async function getAvailableTools() {
    if (_cachedTools) return _cachedTools;
    const data = await api('GET', '/tools');
    _cachedTools = (data && data.tools) ? data.tools : [];
    return _cachedTools;
}

function invalidateToolsCache() { _cachedTools = null; }
...
async function showAgentModal(agentId = null) {
    ...
    const toolCheckboxes = toolNames.map(tool => {
...
async function saveAgent(agentId) {
    ...
    const allTools = await getAvailableTools();
    const enabledTools = allTools.map(t => t.name).filter(tool => {
        const el = document.getElementById(`agent-tool-${tool}`);
...
async function loadTools() {
    const container = document.getElementById('tools-list-content');
    if (!container) return;
    container.innerHTML = '<div class="empty-state"><p>Loading tools...</p></div>';

    invalidateToolsCache();
    const tools = await getAvailableTools();
    if (!tools || tools.length === 0) {
        container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">...</div><p>No tools registered. Start the server with at least one agent to populate the tool registry.</p></div>';
        return;
    }
    ...
    html += `<h3 style="margin: 0 0 0.75rem 0; font-size: 1rem; color: #44403c;">Built-in Tools (${builtIn.length})</h3>`;
    html += '<div class="tools-table-wrap"><table class="tools-table"><thead><tr><th>Name</th><th>Description</th><th>Parameters</th></tr></thead><tbody>';
    ...
    if (extensions.length > 0) {
        html += `<h3 style="margin: 1.5rem 0 0.75rem 0; font-size: 1rem; color: #44403c;">Extensions (${extensions.length})</h3>`;
        html += '<div class="tools-table-wrap"><table class="tools-table"><thead><tr><th>Name</th><th>Description</th><th>Source</th><th>Parameters</th></tr></thead><tbody>';
        for (const t of extensions) {
            const src = t.source_file || '-';
            html += `<tr>
                <td><code>${t.name}</code> <span class="tool-badge extension">ext</span></td>
                <td>${_truncDesc(t.description)}</td>
                <td><code style="font-size: 0.8em;">${src}</code></td>
                <td>${_paramsSummary(t)}</td>
            </tr>`;
        }
        html += '</tbody></table></div>';
    }
...
function _truncDesc(desc) {
    if (!desc) return '-';
    return desc.length > 120 ? desc.substring(0, 117) + '...' : desc;
}

function _paramsSummary(t) {
    if (!t.parameters || t.parameters.length === 0) return '<span style="color: #a8a29e;">none</span>';
    const required = t.parameters.filter(p => p.required !== false);
    const optional = t.parameters.filter(p => p.required === false);
    let parts = [];
    if (required.length) parts.push(`${required.length} required`);
    if (optional.length) parts.push(`${optional.length} optional`);
    const summary = parts.join(', ');
    const details = t.parameters.map(p => {
        const req = p.required !== false ? '*' : '';
        const enumStr = p.enum ? ` [${p.enum.join('|')}]` : '';
        return `${p.name}${req}: ${p.type}${enumStr}`;
    }).join('\n');
    return `<span title="${details.replace(/"/g, '&quot;')}" style="cursor: help; border-bottom: 1px dotted #a8a29e;">${summary}</span>`;
}
