...
class LoopResult:
    ...
    status: Literal["completed", "timeout", "max_turns", "error", "loop_detected", "escalation_needed", "cancelled", "suspended"]
    turns: List[Turn]
    ...
    metadata: Optional[Dict] = None  # Extra data for callers (e.g., atomic_state on suspension)
...
_TOOL_SHORT_HINTS = {
    ...
    "delegate_to_agent": "Assign a task to another agent (suspends your plan until they respond)",
    "delegate_to_human": "Delegate a task to a human team member (suspends your plan until they respond)",
}
...
class AtomicState:
    ...
    @classmethod
    def from_dict(cls, data: dict) -> "AtomicState":
        """Restore from a previously serialized state dict."""
        return cls(
...
class ContextBuilder:
    ...
    def build(
        ...
        Args:
            ...
            traits_prompt: Optional behavioral trait instructions
...
class AgenticLoop:
    ...
    def execute(
        ...
        _restore_plan: Optional[Dict] = None,
        ...
        # Restore saved plan/state from a suspended delegation (must come
        # AFTER _init_execution because that calls planning_manager.reset())
        if _restore_plan:
            plan = ExecutionPlan.from_dict(_restore_plan)
            exec_state["plan"] = plan
            self.planning_manager.current_plan = plan
        if _restore_state:
            # Will be applied below after atomic_state is initialized
            pass
        ...
        # Initialize atomic state (restore from delegation if available)
        if _restore_state:
            atomic_state = AtomicState.from_dict(_restore_state)
        else:
        ...
        elif _restore_plan and exec_state["plan"]:
            # Restored plan — always inject context even if config says otherwise
            plan_context_str = self.planning_manager.get_current_step_context() or ""
        ...
        for turn_number in range(1, self.max_turns + 1):
            ...
            # Check for plan suspension signal (delegation tool)
            if tool_result.metadata and tool_result.metadata.get("action") == "suspend_plan":
                exec_state["pending_actions"] = list(atomic_state.pending_actions)
                loop_result = self._build_loop_result(
                    "suspended", exec_state,
                    final_response="Plan suspended - task delegated to another agent.",
                )
                loop_result.metadata = {"atomic_state": atomic_state.to_dict()}
                return loop_result
